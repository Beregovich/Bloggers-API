...
export class UsersRepository implements IUsersRepository {
    ...
    async createUser(newUser: UserType): Promise<UserType | null> {
        ...
        const createdUser = await this.usersModel.findOne({"accountData.id": newUser.accountData.id})
        return createdUser
            ? createdUser
            : null
    }
    ...
    async findUserByLogin(login: string): Promise<UserType | null> {
        const user = await this.usersModel.findOne({"accountData.login": login}).lean()
        if (user) {
            return user
        } else return null
    }
    ...
    async findUserByConfirmationCode(code: string): Promise<UserType | null> {
        const user = await this.usersModel.findOne({"emailConfirmation.confirmationCode": code})
        if (user) {
            return user
        } else return null
    }
